...
public class HexColorCodeFormatter extends ColorCodeFormatter {
    ...
    public String format(String string, CommandSender player) {
        Map<Integer,char[]> foundColorCodes = new HashMap<>();
        ...
        for(int index = 0; index < arr.length - HEX_COLOR_CODE_LENGTH + 1; index ++) {
            ...
            if(!SingleColorCodeFormatter.isColorCodeChar(arr[index])) {
            ...
            currentOffset = 2;
            if(!SingleColorCodeFormatter.isColorCodeChar(arr[index+currentOffset])) {
                System.out.println(index + " is no &§");
                continue;
            }
            char[] hex = new char[6];
            ...
            for(int offset = 3; offset <= 13; offset++) {
                ...
                if(offset % 2 == 0) {
                    if(!SingleColorCodeFormatter.isColorCodeChar(arr[index+offset])) {
